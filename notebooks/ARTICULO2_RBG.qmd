...
=======
- ADVERTENCIA a explicitar en el texto: en esta sección BIC y AIC se
  calculan con las funciones base de R, donde un valor MENOR indica
  mejor ajuste. Es la convención opuesta a score() de bnlearn, usada
  en las secciones anteriores. Por lo tanto estos valores sólo se
  comparan entre sí y no con los de las tablas previas. -->
>>>>>>> 3f23e105280590eece2067050bc400fe85468011
...
=======
      caption = "Comparación entre la GBN lineal y el modelo no paramétrico basado en splines. En esta tabla un valor menor indica mejor ajuste.")
>>>>>>> 3f23e105280590eece2067050bc400fe85468011
```
...
<!-- TODO: reportar sin interpretar:
- El modelo no paramétrico obtuvo BIC 17481.12 frente a 17599.70 del
  modelo lineal (diferencia de 118.58 puntos) y AIC 17212.30 frente
  a 17488.62
- Todos los términos suaves resultaron significativos (p < 0.05)
  excepto s(age) en el nodo bp (p = 0.1949)
- Los grados de libertad efectivos oscilan entre 1.636 (bp ~ age) y
...
=======
- El modelo no paramétrico mejoró tanto BIC como AIC, lo que indica
  que la ganancia en ajuste compensó la penalización por complejidad
  adicional. Esto sugiere que varias relaciones entre biomarcadores
  no son estrictamente lineales.

- Usar los edf para argumentar CUÁLES relaciones son no lineales:
  * bu ~ sc alcanza el edf más alto (8.384), lo que apunta a una
    relación marcadamente no lineal entre creatinina y urea
  * pcv ~ hemo (6.609) y pot ~ sc (4.870) también muestran curvatura
    relevante
  * bp ~ age es el único término no significativo (edf 1.636,
    p = 0.1949): la relación entre edad y presión arterial es
    prácticamente lineal en esta muestra, y el spline no aporta
    sobre el modelo lineal

- Trade-off flexibilidad vs. complejidad e interpretabilidad, que es
  lo que pide la rúbrica:
  * A favor: captura relaciones curvas que la GBN lineal no puede
    representar, con mejor ajuste en ambos criterios
  * En contra: el modelo deja de tener coeficientes interpretables
    como "un aumento de una unidad en sc implica X en bu"; el efecto
    ahora es una curva que debe leerse gráficamente
  * En contra: cpquery() de bnlearn sólo opera sobre modelos
    paramétricos, de modo que responder las consultas con el modelo
    no paramétrico exigiría implementar manualmente logic sampling o
    likelihood weighting. Por eso las consultas de este trabajo se
    resolvieron sobre la GBN lineal.

- Discutir la limitación de bp: con nueve valores únicos, ninguna
  técnica flexible puede extraer mucha más estructura de esa variable. -->
>>>>>>> 3f23e105280590eece2067050bc400fe85468011
