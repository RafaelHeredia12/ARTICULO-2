...
* **Superioridad del modelado híbrido (lineal-flexible):** La relajación de la linealidad mediante splines (GAM) incrementó el BIC global en $59.29$ puntos, impulsada en más del $91\%$ por el comportamiento no lineal del eje hematológico (`hemo` y `pcv`). No obstante, la degradación predictiva en nodos con distribuciones fuertemente sesgadas (`sc`) indica que la arquitectura idónea para el diagnóstico de la ERC es de naturaleza híbrida: relaciones flexibles para la respuesta hematológica a la anemia y relaciones lineales para los marcadores metabólicos y vasculares.
